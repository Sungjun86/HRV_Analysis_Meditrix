...
class MainActivity : AppCompatActivity() {
    ...
    private val panTompkinsStartOffset = 50
    ...
    private fun readCsvFile(uri: Uri) {
        ...
        contentResolver.openInputStream(uri)?.use { inputStream ->
            ...
            val panInput = ecgValues.drop(panTompkinsStartOffset)
            if (panInput.isEmpty()) {
                latestRrIntervalsSamples = emptyList()
                panTompkinsTextView.text = getString(R.string.not_enough_data_after_offset)
                processedChart.clear()
                processedChart.invalidate()
                return
            }

            val panTompkinsBaseResult = runPanTompkins(panInput, samplingRateHz = 500)
            val panTompkinsResult = panTompkinsBaseResult.copy(
                peakIndices = panTompkinsBaseResult.peakIndices.map { it + panTompkinsStartOffset }
            )
            ...
            renderChart(
                ...
                entries = panTompkinsResult.integratedSignal.mapIndexed { index, value -> Entry((index + panTompkinsStartOffset).toFloat(), value) },
